...
- **커밋 전 필수 절차: 테스트 코드 작성 완료 후 반드시 다음 순서를 따른다:**
  1. 테스트 코드 작성 완료 및 작업 로그 업데이트
  2. 사용자에게 테스트 코드 검토 요청 ("테스트 코드 작성이 완료되었습니다. 검토 후 승인해주세요.")
  3. 사용자 승인 대기
  4. 사용자 승인 후에만 커밋 수행 (커밋 전에도 "커밋 진행해도 될까요?" 명시적 승인 요청)
  5. 커밋 메시지는 `/.cursor/templates/commit-template.md` 형식 준수 (권장 type: test)
- **절대 자동 커밋 금지:** 사용자 명시적 승인 없이는 어떤 경우에도 git 커밋을 수행하지 않는다.
...
- 테스트 코드 작성 및 검증 완료 후 사용자에게 다음을 명시적으로 알린다:
  - "테스트 코드 작성이 완료되었습니다. 작성된 테스트 파일을 검토해주세요."
  - 작성된 테스트 파일 경로 및 주요 내용 요약 제공
  - 사용자 검토 완료 후 승인 대기
- **커밋 전 필수: Lint 오류 확인 및 해결**
  - `read_lints` 도구로 작성/수정된 테스트 파일의 lint 오류 확인
  - 모든 lint 오류를 수정하고 재확인
  - Lint 오류가 없는 것을 확인한 후에만 커밋 승인 요청 진행
- 사용자가 승인하면 커밋 진행 여부를 다시 한번 확인한다: "커밋 진행해도 될까요?"
- 사용자 최종 승인 후에만 커밋을 수행한다.
- 커밋 메시지는 `/.cursor/templates/commit-template.md` 형식을 준수한다 (권장 type: test).
