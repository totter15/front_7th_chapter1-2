...
- 주요 변경사항 요약: 반복 일정 아이콘 표시 기능을 위한 Integration 테스트 4개 케이스 작성 완료 (RED 상태). `src/__tests__/medium.integration.spec.tsx`에 TC-01 ~ TC-04 추가, 반복 일정 생성 헬퍼 함수 `saveRepeatingSchedule` 추가. 테스트 실행 결과: TC-01, TC-02, TC-04 실패 (반복 아이콘 미구현), TC-03 통과 (일반 일정 아이콘 미표시 정상)
